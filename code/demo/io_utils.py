...
def save_audio(path: str | Path, audio_bct: torch.Tensor, *, sample_rate: int) -> None:
    path_obj = Path(path)
    path_obj.parent.mkdir(parents=True, exist_ok=True)
    audio = _normalize_audio(audio_bct)  # (channels, time)
    try:
        # Prefer soundfile: writes WAV via libsndfile, no torchcodec/ffmpeg needed
        # (torchaudio>=2.9 routes save() through torchcodec, unavailable on CPU Spaces).
        import soundfile as sf

        sf.write(str(path_obj), audio.transpose(0, 1).numpy(), int(sample_rate))
    except Exception:
        torchaudio.save(str(path_obj), audio, sample_rate=int(sample_rate))
